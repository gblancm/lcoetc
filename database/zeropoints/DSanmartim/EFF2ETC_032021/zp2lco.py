...
ascii.write([lamout, zpout], fileout, format='commented_header', names=['lambda', 'zeropoint'], formats={'lambda':'%.3f', 'zeropoint':'%.4f'}, overwrite=True)

#plt.plot(lamout, zpaux[0,:])
plt.plot(lam, zplam, '*', label='data')
plt.plot(lamout, zpout, label='interpolation')
plt.legend(loc='best')
plt.show()
