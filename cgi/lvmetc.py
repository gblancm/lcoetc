...
abmag=float(form.getvalue("abmag"))                              # [mag arcsec2]
filfile=dbdir+'/filters/'+form.getvalue("tempfilter")

elamfile=dbdir+'/zeropoints/LVM_'+form.getvalue("telescope")+'_ELAM.dat'
...
sfile=dbdir+'/sky/LVM_'+form.getvalue("telescope")+'_SKY_'+str(nmoon)+'.dat'
...
klamfile=dbdir+'/sky/LVM_'+form.getvalue("telescope")+'_KLAM.dat'

# Read parameter file for TELESCOPE
paramfile=dbdir+'/instruments/LVM_'+form.getvalue("telescope")+'_PARAM.dat'
...
# debugging GB
#mysel=(lam>6500)*(lam<6600)
#ascii.write([lam[mysel], olam[mysel]], tmpdir+'/junk.dat', overwrite=True)
...
hdul.writeto(tmpdir+'/lvmetc_out.fits', clobber=1)
...
# ================================
# Write HTML webpage with results
#=================================
...
print("<title>LVM ETC</title>")
print("</head>")

print("<body>")

print('<center>')
print("<H1>AS4 Local Volume Mapper Exposure Time Calculator</H1>")
print('</center>')
print("<hr>")
...
print("<hr>")
print("2017-08-08: This is a BETA Version written by Guillermo A. Blanc. It has not been validated. Use at your own risk.")
